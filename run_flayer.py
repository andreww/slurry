...
def main(parameters, filename=None):
    ...
    print(
        f"Chemical diffusivity: {data['chemical_diffusivity']:.3g}, kinematic viscosoity: {data['kinematic_viscosity']:.2e}"
    )
    ...
    if data["growth_rate"] > 1.0e-16:
        max_extra_o = np.max(
            feot.mass_percent_o(data["out_x_points"])
            - feot.mass_percent_o(data["xfe_outer_core"])
        )
        ...
        print(
            f"Particle radii between {max_particle_radius:.3g} and {min_particle_radius:.3g} m"
        )
